...
static bool parsePayload(const uint8_t* data, size_t len, SensorReading& out) {
  // Minimum: DEVICE_INFO + PACKET_ID object + BATTERY object = 1 + 2 + 2 bytes
  if (len < 5) {
    return false;
  }
  ...
  while (pos < len) {
    ...
    if (obj_id == BTHOME_OBJ_PACKET_ID) {
      ...
      // [pos+0:2]: temperature_raw (int16, LE), factor 0.01°C
      if (pos + 1 >= len) return false;
      int16_t tempRaw = (int16_t)(data[pos] | (data[pos + 1] << 8));
      out.temperature = tempRaw / 100.0f; // factor 0.01°C
      pos += 2;
    }
    else if (obj_id == BTHOME_OBJ_HUMIDITY) {
      // [pos+0:2]: humidity_raw (uint16, LE), factor 0.01%
      if (pos + 1 >= len) return false;
      uint16_t humidityRaw = (uint16_t)(data[pos] | (data[pos + 1] << 8));
      out.humidity = humidityRaw / 100.0f; // factor 0.01%
      pos += 2;
    }
    else if (obj_id == BTHOME_OBJ_PRESSURE) {
      // [pos+0:3]: pressure_raw (uint24, LE), factor 0.01hPa
      if (pos + 2 >= len) return false;
      uint32_t pressureRaw = (uint32_t)(data[pos] | (data[pos + 1] << 8) | (data[pos + 2] << 16));
      out.pressure = pressureRaw / 100.0f; // factor 0.01hPa
      pos += 3;
    }
    else {
      // Nieznany object ID - pomiń (dla przyszłej rozszerzalności)
      // W rzeczywistości moglibyśmy śledzić długość każdego obiektu,
      // ale BTHome v2 spec określa stałe rozmiary dla znanych obiektów.
      return false;
    }
  }
...
class ScanCallbacks : public NimBLEScanCallbacks {
  void onResult(const NimBLEAdvertisedDevice* device) override {
    ...
    // Deduplikacja: ten sam transaction_id od tego samego MAC = retransmisja
    // tego samego pomiaru (redundancja pakietow adv po stronie sensora),
    // nie nowa probka - pomijamy, zeby nie odswiezac ekranu bez potrzeby.
    ...
    // TODO integracja z UI: tutaj (poza sekcja krytyczna) wywolasz np.
    //   updateSensor(slot, sensors[slot].temperature, sensors[slot].pressure);
    // z modulu wyswietlacza, zeby odswiezyc ekran natychmiast po nowych danych.
  }
};
...
void bleScanInit() {
  ...
  NimBLEDevice::init("");   // pusta nazwa - to my jestesmy Central (Host), nie advertisujemy

  NimBLEScan* pScan = NimBLEDevice::getScan();

  // wantDuplicates=true: chcemy widziec KAZDY pakiet adv, rowniez retransmisje
  // o tym samym payloadzie - dedupikacje robimy sami po transaction_id wyzej,
  // zeby nie polegac na filtrze kontrolera, ktory moglby ukryc przed nami
  // pakiety przydatne do potwierdzenia "sensor wciaz zyje".
